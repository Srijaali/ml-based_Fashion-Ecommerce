...
──────────────────────────────────────────────
  ...
  Steps:
    ...
    3. Copy entire cf_train_kaggle.py script
    4. Select "GPU" (right sidebar)
    5. Run all cells
    6. Download 6 parquet files from /kaggle/working/
...
────────────────────────────────────────
  ...
  If quality score < 80:
    1. Edit cf_train_experiment.py (lines 30-40)
    2. Change N_COMPONENTS, SVD_ITERATIONS, etc.
    3. Run: python test_cf_rapid.py --components=50
    4. Check results in 35 seconds
    5. When happy, repeat Kaggle training
